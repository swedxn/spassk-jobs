...
test('минималистичный интерфейс использует собственный бренд и не возвращает лишние подписи', () => {
  ...
  assert.doesNotMatch(app, /Зарплата указана/);
  assert.match(app, /Источник временно недоступен\. Проверьте актуальность вакансии\./);
  assert.match(app, /summary-main/);
  assert.match(app, /showNoHigher=\{filters\.noHigher\}/);
  assert.match(app, /showNoHigher && isNoHigherEducationVacancy/);
});
...
test('общий каталог фильтрует вакансии без требования высшего образования', () => {
  ...
  assert.deepEqual(vacancyFacts(open), ['Можно без опыта', 'Высшее образование не требуется', 'График: Полный день']);
});
