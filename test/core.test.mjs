...
import { cleanGdeRabotaDescription } from '../src/gderabota-clean.mjs';
...
import { cleanFarpostAddress, cleanFarpostDescription } from '../src/farpost-clean.mjs';
import { jobDateValue, parseFarpostPublishedAt, parseSourceDate, publicationInfo } from '../src/date.mjs';
import { sanitizeTracker } from '../src/tracker.mjs';
import { sourceLinkLabel } from '../src/source-link.mjs';
...
test('другие города и вахта отклоняются', () => {
  ...
  assert.equal(classifyLocation({...local,description:'Работа в Спасске-Дальнем, без переезда, не вахта'}).accepted,true);
  ...
  assert.equal(classifyLocation({...local,source:'FarPost',name:'Водитель самосвала (вахта)',description:'ООО Тест'}).accepted,false);
  assert.equal(classifyLocation({...local,source:'FarPost',name:'Разнорабочий (с. Андреевка, п. Ливадия)',description:'Хасанский район'}).accepted,false);
  assert.equal(classifyLocation({...local,source:'FarPost',name:'Машинист',description:'Хабаровский край, Нанайский район, село Малмыж'}).accepted,false);
  assert.equal(classifyLocation({...local,source:'FarPost',name:'Мастер дорожный на Корфовскую',description:'ОАО РЖД'}).accepted,false);
...
test('дедупликация не зависит от id источника', () => {
  ...
  assert.equal(dedupe([normalizeVacancy({...local,id:'3',name:'Машинист крана от 85 000р'}),normalizeVacancy({...local,id:'4',name:'Машинист крана'})]).length,1);
});
...
test('небольшая почасовая или сменная оплата не теряется', () => {
  assert.equal(normalizeSalaryText('100 – 5 000 ₽'), '100–5 000 ₽');
  assert.equal(normalizeSalaryText('600 ₽ за час'), '600 ₽ за час');
  assert.equal(normalizeSalaryText('500 ₽ /час'), '500 ₽ за час');
  const row=normalizeVacancy({...local,salary:'600 ₽'});
  assert.match(row.warnings.join(' '),/период оплаты/iu);
});

test('повторяющиеся предупреждения удаляются при нормализации', () => {
  const row=normalizeVacancy({...local,warnings:['Проверить','Проверить']});
  assert.deepEqual(row.warnings,['Проверить']);
});

test('импорт трекера отбрасывает повреждённые поля и ограничивает заметку', () => {
  const tracker=sanitizeTracker({one:{favorite:true,status:'интересно',note:'x'.repeat(6000),admin:true},two:'bad',three:{status:'взломано'}});
  assert.deepEqual(Object.keys(tracker),['one']);
  assert.equal(tracker.one.note.length,5000);
  assert.equal(tracker.one.admin,undefined);
});
...
test('непрямая ссылка HH честно подписывается как источник', () => {
  const generic=normalizeVacancy({...local,url:'https://spassk-far.hh.ru/employer/1025275'});
  assert.equal(sourceLinkLabel(generic),'Открыть источник');
  assert.match(generic.warnings.join(' '),/Прямая карточка/iu);
  assert.equal(sourceLinkLabel({...generic,url:'https://spassk-far.hh.ru/vacancy/123'}),'Открыть оригинал');
});
...
test('локальный Telegram-канал принимает только пост с точным Спасском и ролью', () => {
  ...
  const dedicated=parseTelegramMessages(block(3,'Требуется продавец в магазин, 60 000 ₽'),'Vakansii_Spasske_dalnem_RF',Date.parse('2026-06-30T00:00:00Z'));
...
test('ГдеРабота преобразует публичную городскую карточку', () => {
  ...
  assert.doesNotMatch(rows[0].description,/^Воспитатель\s+42 000/iu);
});

test('ГдеРабота убирает повтор заголовка, зарплаты, даты и графика из описания', () => {
  assert.equal(cleanGdeRabotaDescription('Кассир 50 000 - 60 000 ₽ 1 июля 2026, 09:20 Без опыта Полная занятость Полный день Работа за кассой','Кассир'),'Работа за кассой');
});

test('FarPost очищает обломки HTML, дату и счётчик просмотров из описания', () => {
  assert.equal(
    cleanFarpostDescription('₽ " > 60 000 – 90 000 ₽ Грузчик-комплектовщик (г. Спасск-Дальний) ООО Ратимир. Улица Маяковского 2 19 июня 5234','Грузчик-комплектовщик (г. Спасск-Дальний)'),
    'ООО Ратимир. Улица Маяковского 2',
  );
  assert.equal(
    cleanFarpostDescription('₽ " > 44 000 – 52 000 ₽ Требуются повара, помощник повара, зарплата высокая, выплата каждую неделю Ип ким. Деповская 3 21 июня 273','Повар'),
    'Требуются повара, помощник повара, зарплата высокая, выплата каждую неделю Ип ким. Деповская 3',
  );
  assert.equal(
    cleanFarpostDescription('₽ " > 60 000 – 100 000 ₽ Оператор РБУ ООО Железобетон. Улица Краснознамённая 50а вчера в 09:51 600','Оператор РБУ'),
    'ООО Железобетон. Улица Краснознамённая 50а',
  );
  assert.equal(cleanFarpostDescription('4 500 ₽ Фармацевт ООО Энергия. Хасанский район сегодня в 14:19','Фармацевт'),'ООО Энергия. Хасанский район');
  assert.equal(cleanFarpostAddress('Спасск-Дальний, Спасск Дальний, улица Парковая 29 24 июня 708'),'Спасск-Дальний, улица Парковая 29');
  assert.equal(cleanFarpostAddress('Спасск-Дальний, Спасск Дальний ул. 3я Загородная 19 оф 1 вчера в 23'),'Спасск-Дальний, ул. 3я Загородная 19 оф 1');
  assert.equal(
    normalizeVacancy({...local,name:'Оператор РБУ',source:'FarPost',description:'₽ " > 60 000 – 100 000 ₽ Оператор РБУ ООО Железобетон. Улица Краснознамённая 50а вчера в 09:51 600'}).description,
    'ООО Железобетон. Улица Краснознамённая 50а',
  );
});
...
test('прямой сайт Спасского завода ЖБИ импортирует актуальные вакансии', () => {
  ...
  assert.equal(rows.length,2); assert.equal(rows[0].salary,'От 50 000₽'); assert.equal(rows[1].name,'Машинист мостового крана (Крановщик)'); assert.match(rows[0].address,/Краснознаменная/);
});
