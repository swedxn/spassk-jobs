...
export function parseFarpost(html, now = new Date()) {
  ...
  for (const match of html.matchAll(rx)) {
    ...
    const address = cleanFarpostAddress(context.match(/(?:Спасск[\s‑–—-]*Дальн(?:ий|его|ем)[^,.]{0,20}[,.]?\s*)?(?:ул(?:ица)?\.?|пер(?:еулок)?\.?|проспект|микрорайон)\s+[А-ЯЁа-яё0-9\s‑–—-]+(?:,?\s*\d+[а-яА-ЯёЁ\/]*)?/u)?.[0]?.trim() || 'Спасск-Дальний');
    const fullAddress=cleanFarpostAddress(`Спасск-Дальний${address === 'Спасск-Дальний' ? '' : ', ' + address}`);
    const schedule=/вахт/iu.test(`${name} ${context}`)?'Вахта':/удал[её]н/iu.test(`${name} ${context}`)?'Удалённо':/посмен/iu.test(context)?'Посменный':/дневн/iu.test(context)?'Дневной':'Не указано';
    rows.push({ id:`farpost-${url.match(/-(\d+)\.html/)?.[1] || Buffer.from(url).toString('base64url').slice(-18)}`, name, employer, salary:salary(context), city:'Спасск-Дальний', address:fullAddress, experience:/без опыта/iu.test(context)?'Без опыта':'Не указано', education:/средн(?:ее|е-специальное)|без высшего/iu.test(context)?'Без высшего образования':'Не указано', schedule, description:cleanFarpostDescription(context,name).slice(0,900), source:'FarPost', url, publishedAt:parseFarpostPublishedAt(context,now), checkedAt:new Date().toISOString() });
  }
